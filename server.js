...
const commands = {
	'broadcast': (msg) => {
		join(msg).then(() => {
			...
			collector.on('message', m => {
				...
				if (broadcastCommands.hasOwnProperty(m.content.toLowerCase().slice(tokens.prefix.length).split(' ')[0])) broadcastCommands[m.content.toLowerCase().slice(tokens.prefix.length).split(' ')[0]](m, broadcast);
			});
	...
	'tune-in': (msg) => {
		...
		if (broadcasts.has(msg.guild.id)) deleteBroadcast(broadcasts.get(msg.guild.id));
		if (msg.guild.broadcast) {
			if(msg.guild.broadcast.guild.id === msg.guild.id) deleteBroadcast(msg.guild.broadcast);
			else msg.guild.broadcast.listeners.delete(msg.guild.id);
