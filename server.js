...
const commands = {
	...
	'join': (msg) => {
		return new Promise((resolve, reject) => {
			...
			if (!voiceChannel || voiceChannel.type !== 'voice') return msg.reply(':x: I couldn\'t connect to your voice channel...');
			voiceChannel.join().then(connection => resolve(connection)).catch(err => reject(err));
		});
	},
	'add': (msg) => {
		let url = msg.content.split(' ')[1];
		if (url == '' || url === undefined) return msg.channel.send(`:x: You must add a YouTube video url, or id after ${tokens.prefix}add`);
